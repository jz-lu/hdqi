...
def simulated_annealing_search(hamiltonian, n, max_sweeps=1000, initial_beta=0.0, beta_step=0.01):
    ...
    return simulator, current_energy
...
def main():
    ...
    # --- Required Argument ---
    parser.add_argument("file", help="Path to the cham_..._..._....tsv file.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Total number of sweeps to perform. (Default: 500)"
    )
    parser.add_argument(
        "--beta-step",
        type=float,
        default=0.01,
        help="Inverse temperature increment per sweep. (Default: 0.01)"
    )
    parser.add_argument(
        '--initial-beta',
        type=float, 
        default=0.0,
        help="Starting inverse temperature beta. (Default: 0.0)"
    )
    
    args = parser.parse_args()

    # 1. Parse the Hamiltonian
    start_time = time.time()
    hamiltonian, n_qubits = parse_hamiltonian(args.file)
    ...
    print(f"Running simulated annealing for {args.max_sweeps} sweeps with beta_step = {args.beta_step:.4f}...")
    start_time = time.time()
    final_state, final_energy = simulated_annealing_search(
        hamiltonian,
        n_qubits,
    ...
    # 3. Print Results
    print(f"\nSearch finished in {search_time:.2f} seconds.")
    final_sat_frac = calculate_sat_frac(len(hamiltonian), final_energy)
    print(f'Final minimized energy: {final_energy:.2f}')
    print(f'Final satisfaction fraction: {final_sat_frac:.5f}')
